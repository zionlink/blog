---
layout: post
title:  "How to set up a privacy oriented Android smartphone"
date:   2019-09-09
excerpt: "For those who are concerned about their privacy being at risk using an Android smartphone. How to setup an Android Smartphone without Google apps."
image: "/images/2019-09-09/privaphone.jpg"
---

## 1st thing. You probably will have to flash a new ROM on your beloved phone. Choose a plain Android distribution.

There are basically two layers of software which you will probably find on your brand new Android phone. They are layered upon the basic Android Operating System and usually they are closed source, so nobody can tell exactly what is running on your phone.

Many Android phones today come packed with 3rd party "goodies" on them.

# OTP with libpam-google-authenticator

Probably the simplest way to implement a second authentication factor is using OTP (One Time Password) with [libpam-google-authenticator](https://github.com/google/google-authenticator-libpam). Since the code is open source we can reasonably trust it.

This setup requires a configuration on the server side and an app on the phone to generate the passwords when needed. I prefer [FreeOTP](https://freeotp.github.io/) in place of the google app which is closed source.

First install necessary software:

```
root@zion:~# apt update; apt install libpam-google-authenticator
```

Let's configure the PAM for ssh. Add the line:
